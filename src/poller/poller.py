...
class Poller():
    ...
    def get_all_open_pull_requests(self):
        all_pulls_list_url = self.get_content_from_url(
            "https://api.github.com/repos/%s/%s/pulls" % (self.git_user, self.repo))
    ...
    def get_comments_for_pull_request(self, pull_request):
        ...
        all_comments = self.get_content_from_url(
            "https://api.github.com/repos/%s/%s/issues/%d/comments" %
            (self.git_user, self.repo, pull_req_number))
        return all_comments
...
#                    config=self.config).tldt()
            ...
            if revised_sha != self.get_head_sha_for_pull_request(pull_request):
                # commits exist after previous test run
